...
export default connect(
    ...
    dispatch => ({
        addTask: (newTask) => {
            apiTask.createTask(newTask).then(task =>
                dispatch({type: 'ADD_TASK', task})
            ).catch(err => console.error(err));;
        },
        loadTasks: () => {
            apiTask.listTasks().then(listTasks =>
                dispatch({type: 'LOAD_TASK_SUCCESS', task: listTasks})
            ).catch(err => console.error(err));
        },
        deleteTask: (taskId) => {
            apiTask.deleteTask(taskId).then(_id =>
                dispatch({type: 'DELETE_TASK', _id})
            ).catch(err => console.error(err));
        },
    }),
)(App)